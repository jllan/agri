...
void FastText::precomputeSentenceVectors(Matrix& sentenceVectors,std::ifstream& in) {
  ...
  while (i < sentenceVectors.m_) {
    ...
    real norm = vec.norm();
    // norm为向量的p范数，p为训练模型时定义的向量的维度，默认为100
...
void FastText::findNNSent(const Matrix& sentenceVectors, const Vector& queryVec,
  ...
  for (int32_t i = 0; i < numSent; i++) {
    ...
    //std::cout << "queryNorm:" << queryNorm << std::endl;
  ...
  while (i < k && heap.size() > 0) {
    ...
    if (it == banSet.end()) {
      std::cout << heap.top().second << " , " << heap.top().first << std::endl;
//      result.open("/home/jlan/Projects/nlp/sent2vec/data/out.txt", std::ios::app);
//      result << heap.top().second << " , " << heap.top().first << std::endl;
//      result.close();
      i++;
    }
    heap.pop();
...
void FastText::nnSent(int32_t k, std::string filename, std::string filename3) {
  std::string sentence;
  ...
  //std::cout << "第一句： " << sentences[0] << std::endl;

  precomputeSentenceVectors(sentenceVectors, in1);   //计算corpora中每一句的向量值
  std::set<std::string> banSet;
  ...
  std::vector<std::string> resultsent;
  std::ifstream in3(filename3);
  while (in3.peek() != EOF) {
        std::getline(in3, sentence3);
        sentences3.push_back(sentence3);
//        std::cout << sentence3 << std::endl;
  }

  for (int32_t i = 0; i < sentences3.size(); i++) {
//  while (true) {
    query.zero();
    dict_->getLine(in3, line, labels, model_->rng);
    buffer.zero();
    for (auto it = line.cbegin(); it != line.cend(); ++it) {
    ...
    query.addVector(buffer, 1.0);  //buffer为line的向量值
    std::set<std::string>::iterator iter;
    for(iter=banSet.begin();iter!=banSet.end();iter++)
    std::cout<<*iter;
    std::cout << sentences3[i] << std::endl;
//    result.open("/home/jlan/Projects/nlp/sent2vec/data/out.txt", std::ios::app);
//    result << std::endl;
//    result << sentences3[i] << std::endl;
//    result.close();
